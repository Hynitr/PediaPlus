	...
	function getAllPageLinks($count,$href) {
		...
		if($pages>1) {
			if($_GET["page"] == 1) 
				...
				$output = $output . '<a class="link first" onclick="latest(\'' . $href . (1) . '\')" >&#8810;</a><a class="link" onclick="latest(\'' . $href . ($_GET["page"]-1) . '\')" >&#60;</a>';
			...
			if(($_GET["page"]-3)>0) {
				if($_GET["page"] == 1)
					...
					$output = $output . '<a class="link" onclick="latest(\'' . $href . '1\')" >1</a>';
			}
			if(($_GET["page"]-3)>1) {
			...
			for($i=($_GET["page"]-2); $i<=($_GET["page"]+2); $i++)	{
				...
				if($_GET["page"] == $i)
					...
					$output = $output . '<a class="link" onclick="latest(\'' . $href . $i . '\')" >'.$i.'</a>';
			}
			...
			if(($pages-($_GET["page"]+2))>0) {
				if($_GET["page"] == $pages)
					...
					$output = $output . '<a class="link" onclick="latest(\'' . $href .  ($pages) .'\')" >' . ($pages) .'</a>';
			}
			
			if($_GET["page"] < $pages)
				$output = $output . '<a  class="link" onclick="latest(\'' . $href . ($_GET["page"]+1) . '\')" >></a><a  class="link" onclick="latest(\'' . $href . ($pages) . '\')" >&#8811;</a>';
			else				
	...
	function getPrevNext($count,$href) {
		...
		if($pages>1) {
			if($_GET["page"] == 1) 
				...
				$output = $output . '<a class="link first" onclick="latest(\'' . $href . ($_GET["page"]-1) . '\')" >Prev</a>';			
			
			if($_GET["page"] < $pages)
				$output = $output . '<a  class="link" onclick="latest(\'' . $href . ($_GET["page"]+1) . '\')" >Next</a>';
			else				
